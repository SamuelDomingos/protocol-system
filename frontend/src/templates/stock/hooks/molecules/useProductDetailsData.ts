...
import { getProductById, findAllBatchesByProduct, getStockMovementsByProduct } from '@/src/lib/api/stock'
...
export function useProductDetailsData(productId?: string) {
  ...
  const fetchProductLocations = useCallback(async (id: string) => {
    try {
      const productLocations = await findAllBatchesByProduct(id)
      if (productLocations) {
  ...
  const fetchProductMovements = useCallback(async (id: string, params?: any) => {
    ...
    try {
      const response = await getStockMovementsByProduct(id, params);
      if (response) {
